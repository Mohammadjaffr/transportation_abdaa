<div class="container-fluid px-3 pt-4">
    <div class="text-center mb-5">
        <div class="position-relative d-inline-block">
            <div class="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm border border-4 border-white" style="width: 110px; height: 110px; font-size: 3rem; margin: 0 auto; background: linear-gradient(135deg, #0d6efd, #0dcaf0);">
                @if($driver && $driver->Picture)
                    <img src="{{ asset('storage/' . $driver->Picture) }}" class="rounded-circle w-100 h-100 object-fit-cover shadow-sm">
                @else
        ...
        <span class="badge bg-primary bg-opacity-10 text-primary px-3 py-2 rounded-pill fw-bold mt-1">حساب سائق نشط</span>
    </div>

    @if($driver)
    <h6 class="fw-bold mb-3 text-secondary px-2 d-flex align-items-center"><i class="fas fa-address-card ms-2 opacity-50"></i> المعلومات الشخصية</h6>
    ...
    @if($driver->bus)
    <h6 class="fw-bold mb-3 mt-4 text-secondary px-2 d-flex align-items-center"><i class="fas fa-bus ms-2 opacity-50"></i> معلومات الحافلة</h6>
    <div class="card shadow-sm border-0 rounded-4 mb-4">
        <div class="card-body p-0">
            <ul class="list-group list-group-flush rounded-4">
                <li class="list-group-item d-flex justify-content-between align-items-center p-4">
    ...
    <div class="d-grid mt-5 mb-5 pb-4">
        <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="btn btn-danger btn-lg rounded-pill shadow-sm fw-bold border-0 py-3 d-flex align-items-center justify-content-center" style="background: linear-gradient(135deg, #ef4444, #dc2626);">
