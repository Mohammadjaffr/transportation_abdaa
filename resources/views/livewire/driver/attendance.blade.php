<div class="container-fluid px-2 py-3">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3 px-2">
        <div>
            ...
            <small class="text-muted"><i class="fas fa-calendar-day me-1"></i> {{ \Carbon\Carbon::parse($date)->locale('ar')->translatedFormat('j F Y') }}</small>
        </div>
    ...
    <div class="row g-2 px-2 mb-3">
        <div class="col-4">
            <div class="bg-success bg-opacity-10 text-success rounded-4 p-2 text-center border border-success border-opacity-25 shadow-sm">
                <small class="d-block fw-bold opacity-75">الحضور</small>
                <span class="fs-5 fw-bolder">{{ $counters['present'] }}</span>
            </div>
        </div>
        <div class="col-4">
            <div class="bg-danger bg-opacity-10 text-danger rounded-4 p-2 text-center border border-danger border-opacity-25 shadow-sm">
                <small class="d-block fw-bold opacity-75">الغياب</small>
                <span class="fs-5 fw-bolder">{{ $counters['absent'] }}</span>
            </div>
        </div>
        <div class="col-4">
            <div class="bg-warning bg-opacity-10 text-warning rounded-4 p-2 text-center border border-warning border-opacity-25 shadow-sm">
    ...
    @if($isLocked)
    <div class="alert alert-danger rounded-4 shadow-sm d-flex flex-column align-items-center text-center p-5 mb-4 mx-2 border-0 bg-danger bg-opacity-10 text-danger">
    ...
    <div class="d-flex gap-2 mb-3 px-2">
        <div class="input-group shadow-sm rounded-4 overflow-hidden">
            <span class="input-group-text bg-white border-end-0 border-0"><i class="fas fa-search text-muted"></i></span>
            <input wire:model.live.debounce.300ms="search" type="text" class="form-control border-start-0 border-0 ps-0 fw-bold" placeholder="بحث عن طالب...">
        </div>
        <button wire:click="prepareAllPresent" class="btn btn-outline-success text-nowrap rounded-4 shadow-sm fw-bold" style="border-width:2px;" {{ $isLocked ? 'disabled' : '' }}>
                ...
                <div class="card-body p-3">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div>
                            ...
                            <small class="text-muted"><i class="fas fa-map-marker-alt text-primary me-1 opacity-75"></i> {{ $student['region'] ?: 'بدون منطقة' }}</small>
                        </div>
                        @if(isset($student['status']))
                            <div class="{{ $student['status'] ? 'text-success bg-white border border-success' : 'text-danger bg-white border border-danger' }} px-3 py-1 rounded-pill shadow-sm">
                                <small class="fw-bold"><i class="fas {{ $student['status'] ? 'fa-check' : 'fa-times' }} me-1"></i> {{ $student['status'] ? 'حاضر' : 'غائب' }}</small>
                            </div>
                        @endif
                    </div>
